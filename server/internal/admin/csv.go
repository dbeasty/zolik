...
func writeReportCSV(w http.ResponseWriter, rep metrics.Report) {
	...
	_ = c.Write([]string{
		...
		"matches_abandoned", "matches_resumed", "matches_deleted", "matches_deleted_by_host",
		"matches_never_started", "completion_rate",
	...
	for _, p := range rep.Buckets {
		_ = c.Write([]string{
			...
			itoa(p.Matches.Abandoned), itoa(p.Matches.Resumed), itoa(p.Matches.Deleted),
			itoa(p.Matches.DeletedByHost), itoa(p.Matches.NeverStarted),
