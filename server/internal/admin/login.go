...
func (h *Handlers) login(w http.ResponseWriter, r *http.Request) {
	...
	if !login.check(body.Username, body.Password) {
		h.deps.Guard.metrics.Add(metrics.AdminLoginDenied, 1)
		slog.Warn("admin sign-in refused", "remote", key)
	...
	h.deps.Guard.metrics.Add(metrics.AdminLoginOK, 1)
	slog.Info("admin signed in", "user", login.Username, "remote", key)
