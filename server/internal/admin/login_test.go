...
func TestPasswordAdminHasFullAccess(t *testing.T) {
	...
	// The account-management routes this was written against are not ported
	// here — see the package comment. What it still asserts is the property
	// that matters: a password sign-in reaches every guarded route, with no
	// account behind it.
	for _, path := range []string{"/admin/api/session", "/admin/api/report", "/admin/api/status"} {
...
func TestMethodsIsPublic(t *testing.T) {
	...
	body := decode(t, rec)
	// "allowList" rather than the "email" this test was written against: the
	// field names the door (an allow-listed account) rather than the medium
	// it happens to sign in through.
	if body["password"] != true || body["allowList"] != true {
