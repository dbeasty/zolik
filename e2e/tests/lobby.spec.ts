...
test.describe('the waiting room', () => {
  test('a waiting player is visible to the host, and inviting them seats and notifies them', async ({
    ...
    try {
      ...
      // The waiting player opens the waiting room first, independently of
      // anything the host does — connecting *is* the whole action.
      ...
      // and the waiting panel reflects that no one is waiting any more.
      await expect(hostPage.getByText(waiter.username)).toBeVisible({ timeout: 10_000 });
      await expect(hostPage.getByTestId('waiting-players-panel')).toContainText(
  ...
  // a stale, inviteable-looking row behind.
  test('a player who leaves the waiting room disappears from the host\'s view', async ({
    browser,
    request,
  }) => {
    const hostCtx = await browser.newContext();
    ...
    try {
      ...
      // The waiting player leaves — closing the tab is the real-world
      // equivalent of navigating away, and is exactly what tears down the
